...
function apixu_weather_logic( $atts ) {
	...
	if (in_array($widget_scheme, [41, 42, 43, 44, 45])) {
		$widget_scheme_group = 'shm_4';
	} elseif (in_array($widget_scheme, [71, 72, 73])) {
		$widget_scheme_group = 'shm_7';
	} elseif (in_array($widget_scheme, [81, 82, 83])) {
		$widget_scheme_group = 'shm_8';
	} elseif (in_array($widget_scheme, [91])) {
	...
	if ($show_link) {
		...
		$rtn .= "<a href=\"http://www.apixu.com/\" target=\"_blank\" title=\"Free weather API\">{$show_link_text}</a>";
		$rtn .= "</div>";
	}
